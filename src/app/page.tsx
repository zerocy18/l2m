...
export default function Home() {
  ...
  return (
    <>
      아이템 검색
      <input name='input_field' value={text} onChange={(e) => setText(e.target.value)} />
      <button
        onClick={() =>
          setPage((prev) => {
            if (prev <= 1) return 1;
      ...
      <button
        onClick={() => {
          searchAPITest(auth, url, {
            search_keyword: text, // API QueryString (“문서” 메뉴 하위의 검색 Parameters 값 참고)
            sale: false,
            page: page,
          }).then((data) => {
